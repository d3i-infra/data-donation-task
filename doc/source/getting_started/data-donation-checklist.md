...
DDP formats will change over time. Organizations or platforms providing you with a DDP are under no obligation to keep the format of their DDP the same. The formats could change during data collection, be mindful of this fact. 
...
Often when requesting a DDP participants have a choice. For example, for YouTube you can request a portion of the DDP in html format or json format. The most human readable format (html) is often the default. Since participants will not always perfectly follow the instructions when requesting DDPs, it is best to make the process as straightforward as possible. We therefore recommend that you target the default DDP formats as much as possible in your extractor functions. This prevents mistakes and keeps the instructions to the participant as simple as possible. Another way of dealing with it is to provide feedback to the participant whenever you detected they submitted the DDP in a format you did not anticipate.


**Make sure your code will not crash**

A crash in your code causes the data donation task to be stuck. The participant will see an error screen with the stacktrace displayed. You don't want this to happen. Careful programming can prevent your code from crashing.
...
Try to catch errors and handle them gracefully so that participants can continue with the donation task, or alternatively, receive a more user-friendly response. For example: "This file format is currently not supported. Please refer to the instructions on how to request the correct format or contact the study administrator for support."


**Make the least amount of assumptions possible about the data in a DDP**

The more assumptions you make about the data the more opportunities your code has to fail. Think about what assumptions you make and how certain you can be that these are universal across participants and stable over the course of your study. Some examples:
...
* Nested dictionaries: Sometimes you are interested in a value in a dict that is heavily nested. Instead of doing `dict["key1"]["key2"]["key3"]["key_that_sometimes_is_called_something_different"]["key_which_value_you_want"]`, you can first denest or flatten the dictionary and then look directly for `"key_which_value_you_want"`. You can find an example [here](https://github.com/d3i-infra/port-vu-pilot/blob/master/src/framework/processing/py/port/helpers.py). Look for `dict_denester` and `find_items`.


**The researcher is responsible for providing you with DDPs and should be knowledgeable about the data**

If you are reading this checklist, chances are you are going to create a data donation task. However, it could be the case that you are not the principal investigator of the project, but an engineer or person with some coding experience helping the researcher out. Some researchers expect you to be the one to have knowledge about a certain DDP they are interested in. Some researchers believe that because you are working with data, you also have that data available to you, know what it contains or whether data is present in a DDP. This is of course not always the case. Communicate clearly to the researcher that they are responsible for the contents of their study. They should know what the data means and that they should tell you what to extract. In some cases the researcher might not have used the platform they are interested in. If that is the case, tell the researcher to start using the platform they are interested in so they can get familiar with the platform's features, its data export procedure and the contents of the acquired DDP, so that they can get in the position to instruct you on what to extract.
